...
const slice_data_articles = createSlice({
  name: 'slice_data_articles',
  initialState: {
    articles_data: [
      {
        id: 1,
  ...
  reducers: {
    OPEN_TEXT(state: ListArticlesPropsTypes, action: PropsLocalType) {
      state.articles_data = state.articles_data
        .map(el => el.id === action.payload ? {...el, is_opened_text: true} : {...el, is_opened_text: false})
    },
    IS_READ_ARTICLE(state: ListArticlesPropsTypes, action: PropsLocalType) {
      // ставим метку, что статья прочитана
      state.articles_data = state.articles_data
        .map(el => el.id === action.payload ? {...el, is_read: true} : el)
    },
    SAVED_NUMBER_SELECTED_ARTICLE(state: ListArticlesPropsTypes) {
      state.number_selected_article = 1
    },
    CHANGE_ORDER_ELEMENTS(state: ListArticlesPropsTypes, action: PropsLocalType) {
      ...
      // 6. обновляю state
      state.articles_data = old_list_articles_data;
    }
  }
});

export default slice_data_articles.reducer;
export const {OPEN_TEXT, IS_READ_ARTICLE, SAVED_NUMBER_SELECTED_ARTICLE, CHANGE_ORDER_ELEMENTS} = slice_data_articles.actions;